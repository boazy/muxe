...
use crate::config::{
    ActionValidator, CompileInput, CompiledConfig, ConfigDocument, ConfigField, ConfigValue,
...
pub(crate) fn compile_effective(
    ...
    if let Some(override_document) = host_override {
        ...
        merge_values(&mut root, override_document.root);
    }
    ...
    let keyboard = compile_keyboard(
        root.field("keyboard").map(|field| &field.value),
        key_capabilities,
    )?;
...
fn validate_native_actions(
    menus: &mut [CompiledMenu],
    action_validator: Option<&dyn ActionValidator>,
) -> Result<(), Vec<ConfigDiagnostic>> {
    let candidates: Vec<_> = menus
        .iter()
        .flat_map(|menu| &menu.bindings)
        .filter_map(|binding| match &binding.action {
            ActionSpec::Native(candidate) => Some(candidate),
            ActionSpec::Portable(_) => None,
        })
        .collect();
    ...
    let validations = action_validator.validate_native_batch(&candidates)?;
    ...
    for binding in menus.iter_mut().flat_map(|menu| &mut menu.bindings) {
        let ActionSpec::Native(candidate) = &binding.action else {
            continue;
        };
        let validation = validations
            .next()
            .expect("validated native batch has the checked candidate count");
        validate_execution(
...
fn compile_keyboard(
    ...
    let Some(value) = value else {
        return Ok(KeyboardProfile::Vt100 {
    ...
    match mode {
        ...
        _ => Err(vec![ConfigDiagnostic::error(
            ...
            value
                .field("mode")
                .map_or_else(|| value.span.clone(), |field| field.value.span.clone()),
        )]),
    }
}
...
fn keyboard_flag(
    ...
    let enabled = expect_bool(
        &field.value,
        format!("`keyboard.kitty.{name}` must be boolean"),
    )?;
...
fn compile_settings(
    ...
    if let Some(execution) = value.field("execution") {
        validate_fields(
            &execution.value,
            &["mode", "timeout", "on-timeout", "on-menu-control"],
        )?;
        if let Some(mode) = execution.value.field("mode") {
            base.execution.mode =
                match expect_string(&mode.value, "execution mode must be a string")? {
                    "await" => ExecutionMode::Await,
                    "detach" => ExecutionMode::Detach,
                    _ => {
                        return Err(vec![ConfigDiagnostic::error(
                            DiagnosticCode::InvalidValue,
                            "execution mode must be `await` or `detach`",
                            mode.value.span.clone(),
                        )]);
                    }
                };
        ...
        if let Some(action) = execution.value.field("on-timeout") {
            base.execution.on_timeout =
                match expect_string(&action.value, "on-timeout must be a string")? {
                    "detach" => TimeoutAction::Detach,
                    "cancel" => TimeoutAction::Cancel,
                    _ => {
                        return Err(vec![ConfigDiagnostic::error(
                            DiagnosticCode::InvalidValue,
                            "on-timeout must be `detach` or `cancel`",
                            action.value.span.clone(),
                        )]);
                    }
                };
        }
        if let Some(action) = execution.value.field("on-menu-control") {
            base.execution.on_menu_control =
                match expect_string(&action.value, "on-menu-control must be a string")? {
                    "detach" => MenuControlAction::Detach,
                    "cancel" => MenuControlAction::Cancel,
                    _ => {
                        return Err(vec![ConfigDiagnostic::error(
                            DiagnosticCode::InvalidValue,
                            "on-menu-control must be `detach` or `cancel`",
                            action.value.span.clone(),
                        )]);
                    }
                };
    ...
    if scope == SettingsScope::Global {
        if let Some(reload) = value.field("reload") {
            ...
            if let Some(watch) = reload.value.field("watch") {
                base.reload.watch =
                    expect_bool(&watch.value, "`settings.reload.watch` must be boolean")?;
            }
            if let Some(debounce) = reload.value.field("debounce") {
                base.reload.debounce =
                    parse_duration_value(&debounce.value, false)?.expect("`off` was rejected");
            }
        }
        if let Some(host) = value.field("host") {
            ...
            if let Some(version) = host.value.field("version") {
                ...
                if let Some(check) = version.value.field("check") {
                    base.host.version_check = match expect_string(
                        &check.value,
                        "`settings.host.version.check` must be a string",
                    )? {
...
fn selected_asset_name(
    root: &ConfigValue,
    field_name: &str,
) -> Result<(String, SourceSpan), Vec<ConfigDiagnostic>> {
...
fn parse_theme_section(value: Option<&ConfigValue>) -> Result<ThemeSection, Vec<ConfigDiagnostic>> {
    let Some(value) = value else {
        return Ok(ThemeSection::default());
    };
            ...
            styles.insert(
                ...
                Style {
                    ...
                    bold: style
                        .value
                        .field("bold")
                        .map(|field| expect_bool(&field.value, "style `bold` must be boolean"))
                        .transpose()?
                        .unwrap_or(false),
                    dim: style
                        .value
                        .field("dim")
                        .map(|field| expect_bool(&field.value, "style `dim` must be boolean"))
                        .transpose()?
                        .unwrap_or(false),
                    italic: style
                        .value
                        .field("italic")
                        .map(|field| expect_bool(&field.value, "style `italic` must be boolean"))
                        .transpose()?
                        .unwrap_or(false),
                    underline: style
                        .value
                        .field("underline")
                        .map(|field| expect_bool(&field.value, "style `underline` must be boolean"))
                        .transpose()?
                        .unwrap_or(false),
                    strikethrough: style
                        .value
                        .field("strikethrough")
                        .map(|field| {
                            expect_bool(&field.value, "style `strikethrough` must be boolean")
                        })
                        .transpose()?
                        .unwrap_or(false),
                },
...
fn parse_color_scheme_document(
...
) -> Result<ColorScheme, Vec<ConfigDiagnostic>> {
    validate_fields(&document.root, &["title", "palette", "colors"])?;
    let title = expect_string(
        &required_field(&document.root, "title")?.value,
    ...
    if let Some(palette_map) = document.root.field("palette") {
        for field in mapping_fields(
            &palette_map.value,
            "color-scheme `palette` must be a mapping",
        )? {
...
fn nonnegative_u16(value: &ConfigValue, name: &str) -> Result<u16, Vec<ConfigDiagnostic>> {
    let ConfigValueKind::Integer(integer) = value.kind else {
        return Err(vec![ConfigDiagnostic::error(
            DiagnosticCode::InvalidValue,
            format!(
                "`{name}` must be a non-negative integer no greater than {}",
                u16::MAX
            ),
            value.span.clone(),
        )]);
...
impl MenuCompiler<'_> {
    ...
    fn compile_binding(
        ...
        let hidden = field
            .value
            .field("hidden")
            .map(|field| expect_bool(&field.value, "`hidden` must be boolean"))
            .transpose()?
            .unwrap_or(false);
        ...
        if !settings.execution_mode_explicit
            && matches!(
                action.kind(),
                ActionKind::Portable(PortableActionKind::CommandExecute)
            )
        {
        ...
        let conditions = compile_conditions(
            ...
            matches!(
                action.kind(),
                ActionKind::Portable(
                    PortableActionKind::MenuPagePrev | PortableActionKind::MenuPageNext
                )
            ),
        )?;
    ...
    #[expect(
        clippy::too_many_lines,
        reason = "the closed portable action grammar stays co-located with its field validation"
    )]
    fn parse_portable(
        &self,
...
fn validate_execution(
    ...
    if policy.execution_timeout_is_cancel() && !capabilities.cancellable {
        return Err(vec![ConfigDiagnostic::error(
            DiagnosticCode::InvalidValue,
            "on-timeout: cancel requires a cancellable action",
            span,
        )]);
    }
    if policy.on_menu_control == MenuControlAction::Cancel && !capabilities.cancellable {
        return Err(vec![ConfigDiagnostic::error(
            DiagnosticCode::InvalidValue,
            "on-menu-control: cancel requires a cancellable action",
            span,
        )]);
    }
    Ok(())
        ...
        ConfigValueKind::Mapping(fields) => {
            let type_field = fields
                ...
                .ok_or_else(|| {
                    vec![ConfigDiagnostic::error(
                        DiagnosticCode::MissingField,
                        "action mapping requires `type`",
                        value.span.clone(),
                    )]
                })?;
            let type_name =
                expect_string(&type_field.value, "action `type` must be a string")?.to_owned();
            let fields = fields
                .iter()
...
fn compact_action_fields(
    ...
    for argument in arguments {
        let (name, value) = if let Some((name, raw_value)) = compact_assignment(argument) {
            ...
            (
                name.to_owned(),
                compact_action_scalar(raw_value, span.clone())?,
            )
        } else {
            if named {
                return Err(vec![ConfigDiagnostic::error(
...
fn compact_scalar(token: &str, span: SourceSpan) -> Result<ConfigValue, Vec<ConfigDiagnostic>> {
    ...
    let document = ConfigDocument::parse(
        SourceId::new("<compact action scalar>"),
        Arc::<str>::from(yaml),
    )
    .map_err(|_| {
        vec![ConfigDiagnostic::error(
    ...
    if matches!(
        value.kind,
        ConfigValueKind::Sequence(_) | ConfigValueKind::Mapping(_)
    ) {
        return Err(vec![ConfigDiagnostic::error(
            DiagnosticCode::InvalidActionArguments,
...
fn compact_action_scalar(
    token: &str,
    span: SourceSpan,
) -> Result<ConfigValue, Vec<ConfigDiagnostic>> {
...
fn compile_conditions(
    value: Option<&ConfigValue>,
    pager: bool,
) -> Result<BindingConditions, Vec<ConfigDiagnostic>> {
    let Some(value) = value else {
        return Ok(BindingConditions::default());
    };
    ...
    let compile = |name: &str| -> Result<Option<ConditionProgram>, Vec<ConfigDiagnostic>> {
        ...
        let program = ConditionProgram::compile(source, field.value.span.clone())
            .map_err(|error| vec![error])?;
...
impl Injection {
    fn parse(field: &ConfigField) -> Result<Self, Vec<ConfigDiagnostic>> {
        ...
        let selector_type = expect_string(
            &required_field(&select.value, "type")?.value,
            "selector type must be string",
        )?;
        let selector_value = select
            .value
            .field("value")
            .map(|field| {
                expect_string(&field.value, "selector value must be string").map(str::to_owned)
            })
            .transpose()?;
        ...
        let defaults = match expect_string(
            ...
            "injection action type must be string",
        )? {
            "override" => false,
            "defaults" => true,
            _ => {
                return Err(vec![ConfigDiagnostic::error(
                    ...
                    "injection action type must be `override` or `defaults`",
                    action.value.span.clone(),
                )]);
            }
        };
...
impl Selector {
    fn parse(
        ...
        match kind {
            ...
            "id:regex" => Regex::new(&require(value)?)
                .map(Self::IdRegex)
                .map_err(|error| {
                    vec![ConfigDiagnostic::error(
                        DiagnosticCode::InvalidInjection,
            ...
            "title:regex" => Regex::new(&require(value)?)
                .map(Self::TitleRegex)
                .map_err(|error| {
                    vec![ConfigDiagnostic::error(
                        DiagnosticCode::InvalidInjection,
        ...
        let tags = menu
            ...
            .and_then(|field| match &field.value.kind {
                ConfigValueKind::Sequence(values) => {
                    Some(values.iter().filter_map(ConfigValue::as_str))
                }
                _ => None,
            });
        match self {
            ...
            Self::TagsContain(expected) => {
                tags.is_some_and(|mut tags| tags.any(|tag| tag == expected))
            }
        }
    }
}

fn apply_injection_to_menus(
    menus: &mut ConfigValue,
    injection: &Injection,
) -> Result<(), Vec<ConfigDiagnostic>> {
...
fn apply_injection_to_menu(
    ...
    if let Some(bindings) = menu.field_mut("bindings")
        && let Some(bindings) = bindings.value.as_mapping_mut()
    {
        for binding in bindings {
            if let Some(action) = binding.value.field_mut("action")
                && let Some(submenu) = action.value.field_mut("submenu")
            {
...
fn merge_defaults(target: &mut ConfigValue, defaults: &ConfigValue) {
    let (Some(target), Some(defaults)) = (target.as_mapping_mut(), defaults.as_mapping()) else {
        return;
    };
...
fn context_reference(
    ...
    ContextReference::parse(path, mapping[0].value.span.clone())
        .map(Some)
        .map_err(|error| vec![error])
}
...
fn action_string_scalar(
    ...
    match &scalar.value.kind {
        ...
        ConfigValueKind::Context(reference) if allowance.accepts(reference.expected_type()) => {
            Ok(scalar)
        }
...
fn action_index_scalar(value: &ConfigValue) -> Result<ActionScalar, Vec<ConfigDiagnostic>> {
    ...
    match &scalar.value.kind {
        ...
        ConfigValueKind::Context(reference)
            if reference.expected_type() == ContextType::UnsignedInteger =>
        {
            Ok(scalar)
        }
        ConfigValueKind::Context(_) => Err(vec![ConfigDiagnostic::error(
    ...
    values
        ...
        .map(|value| {
            ...
            if let ConfigValueKind::String(key) = &scalar.value.kind {
                CanonicalKey::parse_diagnostic(key, scalar.value.span.clone())
                    .map_err(|error| vec![error])?;
            }
            Ok(scalar)
        })
...
fn string_sequence(
    ...
    values
        .iter()
        .map(|value| {
            expect_string(value, format!("{name} values must be strings")).map(str::to_owned)
        })
        .collect()
}
...
fn optional_duration(
    ...
    name: &str,
    off_allowed: bool,
) -> Result<Option<Duration>, Vec<ConfigDiagnostic>> {
    value
        .field(name)
...
fn parse_duration_value(
    ...
    if text == "off" {
        return off_allowed.then_some(None).ok_or_else(|| {
            vec![ConfigDiagnostic::error(
                DiagnosticCode::InvalidValue,
                "`off` is not valid here",
                value.span.clone(),
            )]
        });
    }
    let parse = |suffix: &str, multiplier: u64| {
    ...
    parse("ms", 1)
        ...
        .ok_or_else(|| {
            vec![ConfigDiagnostic::error(
                DiagnosticCode::InvalidValue,
                "duration must use ms, s, m, or off",
                value.span.clone(),
            )]
        })
}
...
fn required_field_mut<'a>(
    ...
    name: &str,
) -> Result<&'a mut ConfigField, Vec<ConfigDiagnostic>> {
    let span = value.span.clone();
    value.field_mut(name).ok_or_else(|| {
        vec![ConfigDiagnostic::error(
            DiagnosticCode::MissingField,
            format!("missing required `{name}`"),
            span,
...
) -> Result<&str, Vec<ConfigDiagnostic>> {
...
fn optional_string<'a>(
    value: &'a ConfigValue,
    name: &str,
) -> Result<Option<&'a str>, Vec<ConfigDiagnostic>> {
    value
        .field(name)
        .map(|field| expect_string(&field.value, format!("`{name}` must be a string")))
        .transpose()
}
...
fn mapping_fields_mut<'a>(
    value: &'a mut ConfigValue,
    message: &str,
) -> Result<&'a mut Vec<ConfigField>, Vec<ConfigDiagnostic>> {
    let span = value.span.clone();
...
fn nearest<'a>(value: &str, options: &'a [&str]) -> Option<&'a str> {
    options
        .iter()
        .copied()
        .min_by_key(|option| edit_distance(value, option))
        .filter(|option| edit_distance(value, option) <= 3)
}
